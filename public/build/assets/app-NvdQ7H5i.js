var e=new class{constructor(){this.baseUrl=`/api`}getCsrfToken(){let e=document.querySelector(`meta[name="csrf-token"]`);return e?e.getAttribute(`content`):``}getHeaders(e=!1){let t={Accept:`application/json`,"X-Requested-With":`XMLHttpRequest`,"X-CSRF-TOKEN":this.getCsrfToken()};return e||(t[`Content-Type`]=`application/json`),t}async handleResponse(e){let t=e.headers.get(`content-type`)?.includes(`application/json`)?await e.json():null;if(!e.ok){let n=t?.message||t?.error||`HTTP error ${e.status}`,r=Error(n);throw r.status=e.status,r.data=t,r}return t}async get(e,t={}){let n=new URL(`${this.baseUrl}${e}`,window.location.origin);Object.keys(t).forEach(e=>{t[e]!==null&&t[e]!==void 0&&n.searchParams.append(e,t[e])});let r=await fetch(n.toString(),{method:`GET`,headers:this.getHeaders(),credentials:`include`});return this.handleResponse(r)}async post(e,t={},n=!1){let r=`${this.baseUrl}${e}`,i={method:`POST`,headers:this.getHeaders(n),credentials:`include`,body:n?t:JSON.stringify(t)},a=await fetch(r,i);return this.handleResponse(a)}async put(e,t={},n=!1){let r=`${this.baseUrl}${e}`,i={method:`PUT`,headers:this.getHeaders(n),credentials:`include`,body:n?t:JSON.stringify(t)},a=await fetch(r,i);return this.handleResponse(a)}async delete(e){let t=`${this.baseUrl}${e}`,n=await fetch(t,{method:`DELETE`,headers:this.getHeaders(),credentials:`include`});return this.handleResponse(n)}},t=class{constructor(e={}){this.videoElement=e.videoElement||document.querySelector(`#ar-video`),this.canvasElement=e.canvasElement||document.createElement(`canvas`),this.stream=null,this.isScanning=!1}async init(){if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia){console.warn(`Akses kamera (getUserMedia) membutuhkan koneksi HTTPS atau host localhost.`);let e=document.querySelector(`#camera-warning`);e&&(e.classList.remove(`hidden`),e.textContent=`Kamera AR memerlukan protokol HTTPS (atau http://localhost:8000) sesuai aturan keamanan browser.`);return}try{this.videoElement&&(this.stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:`environment`,width:{ideal:1280},height:{ideal:720}},audio:!1}),this.videoElement.srcObject=this.stream,await this.videoElement.play())}catch(e){console.warn(`Gagal mengakses kamera:`,e)}}fetchCurrentLocation(){return new Promise(e=>{if(typeof navigator>`u`||!navigator.geolocation||typeof navigator.geolocation.getCurrentPosition!=`function`)return e({latitude:null,longitude:null});navigator.geolocation.getCurrentPosition(t=>{e({latitude:t.coords.latitude,longitude:t.coords.longitude})},t=>{console.warn(`Lokasi GPS tidak dapat diakses saat capture:`,t.message),e({latitude:null,longitude:null})},{enableHighAccuracy:!1,timeout:4e3})})}captureFrame(){if(!this.videoElement||this.videoElement.readyState!==4)throw Error(`Kamera belum siap atau tidak diizinkan di asal koneksi non-HTTPS.`);let e=this.videoElement.videoWidth||640,t=this.videoElement.videoHeight||480;return this.canvasElement.width=e,this.canvasElement.height=t,this.canvasElement.getContext(`2d`).drawImage(this.videoElement,0,0,e,t),this.canvasElement.toDataURL(`image/jpeg`,.85)}async performScan(){if(!this.isScanning){this.isScanning=!0;try{let t=this.captureFrame(),n=await this.fetchCurrentLocation(),r={image_base64:t,latitude:n.latitude,longitude:n.longitude},i=await e.post(`/scan`,r);return this.isScanning=!1,i}catch(e){throw this.isScanning=!1,e}}}stop(){this.stream&&=(this.stream.getTracks().forEach(e=>e.stop()),null)}},n=class{constructor(e={}){this.containerElement=e.containerElement||document.querySelector(`#gallery-container`),this.modalElement=e.modalElement||document.querySelector(`#gallery-modal`),this.progressTextElement=e.progressTextElement||document.querySelector(`#seedex-progress-text`),this.progressBarElement=e.progressBarElement||document.querySelector(`#seedex-progress-bar`),this.items=[],this.speciesCatalog=[],this.role=window.USER_ROLE||`viewer`}async loadGallery(){try{let t=await e.get(`/gallery`);this.items=t.data?.data||t.data||[],this.role=t.data?.role||t.role||this.role,this.totalSpeciesCount=t.data?.total_species||t.total_species||0;try{let t=await e.get(`/ranger/species`);this.speciesCatalog=t.data?.data||t.data||[]}catch{this.speciesCatalog=[]}this.render()}catch(e){console.error(`[GalleryModule] Gagal memuat Seedex:`,e)}}render(){if(!this.containerElement)return;let e=[`ranger`,`admin`].includes(this.role),t=this.speciesCatalog.length,n=e?this.items.length:t>0?t:this.totalSpeciesCount||this.items.length,r=this.items.length,i=e?r>0?100:0:n>0?Math.min(Math.round(r/n*100),100):0;if(this.progressTextElement)if(e){let e=window.translations?.ranger_uploaded||`Spesimen Di-upload`;this.progressTextElement.textContent=`${r} ${e}`}else{let e=window.translations?.discovered||`Ditemukan`;this.progressTextElement.textContent=`${r} / ${n} Seedex ${e}`}if(this.progressBarElement&&(this.progressBarElement.style.width=`${i}%`),this.items.length===0&&(e||this.speciesCatalog.length===0)){let t=e?window.translations?.ranger_empty_title||`Belum Ada Temuan Di-upload`:window.translations?.empty_gallery||`Seedex Masih Kosong`,n=e?window.translations?.ranger_empty_subtitle||`Gunakan Peta Digital atau Kamera AR untuk mulai mendokumentasikan tumbuhan di lapangan!`:window.translations?.empty_gallery_subtitle||`Jelajahi peta dan temukan marker spesies tumbuhan untuk mengumpulkan entri ke Seedex!`,r=window.translations?.open_map_btn||`Buka Peta Temuan`;this.containerElement.innerHTML=`
        <div class="card-gg p-12 text-center space-y-4 max-w-md mx-auto">
          ...
          <a href="/peta" class="btn-gg-primary inline-flex items-center gap-2 text-xs">${r}</a>
        </div>
      `;return}let a=new Set,o=new Map;this.items.forEach(e=>{let t=e.sighting?.species||e.species||e.plant_species;t?.id&&(a.add(t.id),o.set(t.id,e))});let s=`<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">`,c=window.translations||{};if(this.items.forEach(e=>{let t=e.sighting||e,n=t.species||e.plant_species,r=n?.common_name||`Spesimen Flora`,i=n?.scientific_name||``,a=t.photo_url||e.photo_url||n?.reference_image_url||`/images/logo-plantGuardian.jpeg`,o=n?.conservation_status||`Common`,l=this.formatRarityBadge(o),u=`#9E9E8A`;o.toLowerCase().includes(`rare`)||o.toLowerCase().includes(`rentan`)?u=`#7D5BA6`:(o.toLowerCase().includes(`uncommon`)||o.toLowerCase().includes(`lindung`))&&(u=`#4C8C4A`),s+=`
        <div class="seedex-card card-gg card-gg-hover p-3 rounded-2xl overflow-hidden cursor-pointer group" data-id="${e.id}">
          <div class="h-36 rounded-xl bg-[#E2E1C4] overflow-hidden relative mb-2.5">
            <img src="${a}" onerror="this.onerror=null; this.src='/images/logo-plantGuardian.jpeg';" alt="${r}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
            <span class="absolute top-2 right-2 text-[10px] font-baloo font-extrabold px-2 py-0.5 rounded-full text-[#FBFAF0] uppercase shadow-xs" style="background-color: ${u};">
              ${l}
            </span>
          </div>
          <div>
            <h4 class="font-baloo font-bold text-sm text-[#1F3D20] leading-tight truncate group-hover:text-[#4C8C4A] transition-colors">${r}</h4>
            <p class="font-nunito text-[11px] text-[#6B6B55] italic truncate">${i||c.discovered||`Ditemukan`}</p>
          </div>
        </div>
      `}),!e){let e=Math.max(0,n-r),t=c.locked_card_text||`Belum Ditemukan`;for(let n=0;n<e;n++)s+=`
          <div class="card-gg p-3 rounded-2xl opacity-60 bg-[#E2E1C4]/40 flex flex-col justify-between">
            ...
            <div>
              ...
              <p class="font-nunito text-[11px] text-[#6B6B55]">🔒 ${t}</p>
            </div>
          </div>
        `}s+=`</div>`,this.containerElement.innerHTML=s,this.containerElement.querySelectorAll(`.seedex-card`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-id`);this.openDetail(t)})})}formatRarityBadge(e){let t=window.translations||{},n=(e||`Common`).toLowerCase();return n.includes(`vulnerab`)||n.includes(`rentan`)?t.status_vulnerable||`RENTAN`:n.includes(`endanger`)||n.includes(`terancam`)?t.status_endangered||`TERANCAM PUNAH`:n.includes(`protect`)||n.includes(`lindung`)?t.status_protected||`DILINDUNGI`:n.includes(`least`)||n.includes(`rendah`)?t.status_least_concern||`RISIKO RENDAH`:t.status_common||`UMUM`}async openDetail(t){try{let n=await e.get(`/gallery/${t}`),r=n.data?.data||n.data;if(!r)return;let i=r.sighting||r,a=i.species||r.plant_species,o=a?.common_name||`Hasil Temuan`,s=a?.scientific_name||``,c=a?.description||`Deskripsi tanaman belum tersedia.`,l=a?.care_instructions||``,u=i.photo_url||r.photo_url||a?.reference_image_url||`/images/logo-plantGuardian.jpeg`;if(this.modalElement){this.modalElement.querySelector(`#modal-title`).textContent=o,this.modalElement.querySelector(`#modal-scientific`).textContent=s;let t=this.modalElement.querySelector(`#modal-img`);t&&(t.onerror=function(){this.onerror=null,this.src=`/images/logo-plantGuardian.jpeg`},t.src=u),this.modalElement.querySelector(`#modal-desc`).textContent=c;let n=this.modalElement.querySelector(`#modal-care-text`);n&&(n.textContent=l||`Belum ada petunjuk perawatan dari Ranger. Menyiram 2x sehari dan memberi kompos teratur disarankan.`);let i=this.modalElement.querySelector(`#toggle-care-edit-btn`),d=this.modalElement.querySelector(`#care-edit-form`),f=this.modalElement.querySelector(`#care-instructions-input`);if(d&&d.classList.add(`hidden`),this.role===`ranger`&&i&&a?.id){i.classList.remove(`hidden`),i.onclick=()=>{f&&(f.value=a.care_instructions||``),d&&d.classList.toggle(`hidden`)};let t=this.modalElement.querySelector(`#cancel-care-edit-btn`);t&&(t.onclick=()=>d.classList.add(`hidden`));let r=this.modalElement.querySelector(`#save-care-edit-btn`);r&&(r.onclick=async()=>{try{r.disabled=!0,r.textContent=`Menyimpan...`;let t=f.value.trim();await e.put(`/ranger/species/${a.id}`,{species_code:a.species_code,common_name:a.common_name,description:a.description,care_instructions:t}),a.care_instructions=t,n&&(n.textContent=t||`Belum ada petunjuk perawatan dari Ranger.`);let i=window.translations||{};d&&d.classList.add(`hidden`),alert(i.instructions_updated||`✨ Petunjuk perawatan pohon berhasil diperbarui!`)}catch(e){alert(`Gagal menyimpan petunjuk perawatan: `+(e.message||`Terjadi kesalahan`))}finally{r.disabled=!1,r.textContent=window.translations&&window.translations.save_instructions||`Simpan Petunjuk`}})}else i&&i.classList.add(`hidden`);let p=this.modalElement.querySelector(`#modal-delete-btn`);p&&(p.onclick=()=>this.deleteItem(r.id)),this.modalElement.classList.remove(`hidden`)}}catch(e){console.error(`[GalleryModule] Gagal memuat detail Seedex:`,e)}}async deleteItem(t){if(confirm(`Apakah kamu yakin ingin menghapus temuan ini dari Seedex?`))try{await e.delete(`/gallery/${t}`),this.modalElement&&this.modalElement.classList.add(`hidden`),await this.loadGallery()}catch{alert(`Gagal menghapus temuan.`)}}},r=class{constructor(e={}){this.containerElement=e.containerElement||document.querySelector(`#garden-plots-container`),this.plots=[],this.seeds=[],this.userCoin=0,this.userExp=0,this.updateTimer=null,this.selectedPlotForPlanting=null}async init(){await this.loadPlots(),this.startAutoRefresh()}startAutoRefresh(){this.updateTimer&&clearInterval(this.updateTimer),this.updateTimer=setInterval(()=>this.tickGrowthProgress(),1e3)}tickGrowthProgress(){let e=!1,t=Date.now();this.plots.forEach(n=>{let r=n.current_planting;if(!r||r.status!==`growing`)return;let i=r.planted_at?new Date(r.planted_at).getTime():t,a=r.ready_at?new Date(r.ready_at).getTime():t,o=Math.max(1,a-i),s=Math.max(0,t-i),c=Math.max(0,Math.ceil((a-t)/1e3));if(c<=0){r.status=`ready`,e=!0;return}let l=Math.min(99,Math.max(1,Math.floor(s/o*100))),u=this.containerElement?.querySelector(`[data-plot-slot="${n.slot_number}"]`);if(!u)return;let d=u.querySelector(`.growth-bar-fill`);d&&(d.style.width=`${l}%`);let f=u.querySelector(`.growth-countdown`);f&&(f.textContent=this.formatTimeRemaining(c));let p=u.querySelector(`.growth-time-pill`);p&&(p.textContent=`⏳ ${this.formatTimeRemaining(c)}`);let m=u.querySelector(`.growth-stage-label`);m&&(l<35?m.textContent=`Tunas Mungil (${l}%)`:l<75?m.textContent=`Tumbuh Berdaun (${l}%)`:m.textContent=`Hampir Matang (${l}%)`)}),e&&this.loadPlots()}async loadPlots(){try{let t=await e.get(`/minigame/plots`);this.plots=t.data||[],this.seeds=t.seeds||[],this.tools=t.tools||[],this.userCoin=t.user_coin||0,this.userExp=t.user_exp||0,typeof window.updateUserCoin==`function`&&window.updateUserCoin(this.userCoin),typeof window.updateUserExp==`function`&&window.updateUserExp(this.userExp),this.render()}catch(e){console.error(`Gagal memuat lahan kebun:`,e),this.containerElement&&(this.containerElement.innerHTML=`
          <div class="text-center py-12 text-[#F5F4DA]">
            <p>Gagal memuat lahan kebun. Silakan muat ulang halaman.</p>
          </div>
        `)}}showToast(e,t=`success`){typeof window.showToast==`function`?window.showToast(e,t):alert(e)}getSeedDetails(e){let t=window.translations&&window.translations.title&&window.translations.title.includes(`Virtual Garden`)||document.documentElement.lang===`en`;return{seed_sunflower:{name:t?`Sunflower`:`Bunga Matahari`,icon:`🌻`,duration:30,exp:50,coin:70,price:50},seed_tomato:{name:t?`Organic Tomato`:`Tomat Organik`,icon:`🍅`,duration:60,exp:90,coin:110,price:75},seed_monstera:{name:`Monstera Deliciosa`,icon:`🌿`,duration:105,exp:160,coin:180,price:120},seed_orchid:{name:t?`Black Orchid`:`Anggrek Hitam`,icon:`🪻`,duration:180,exp:300,coin:310,price:200},SEED_DEFAULT:{name:t?`Sunflower`:`Bunga Matahari`,icon:`🌻`,duration:30,exp:50,coin:70,price:50}}[e]||{name:t?`Species Seed`:`Benih Spesies`,icon:`🌱`,duration:30,exp:50,coin:70,price:50}}formatTimeRemaining(e){let t=window.translations&&window.translations.title&&window.translations.title.includes(`Virtual Garden`)||document.documentElement.lang===`en`;if(e<=0)return t?`Ready to harvest!`:`Siap panen!`;let n=Math.floor(e/60),r=e%60;return n>0?`${n}m ${r}s`:`${r}s`}renderPlantVisual(e,t,n){return e?e.status===`ready`||e.ready_at&&new Date>=new Date(e.ready_at)?`
    ...
    `;try{let e=await this.getLeaderboardHistory(),t=e&&e.data?e.data:[];if(t.length===0){this.historyContainer.innerHTML=`
      ...
      `}}}},s=class{constructor(e,t={}){this.mapContainerId=e,this.userRole=(t.userRole||`viewer`).toLowerCase(),this.map=null,this.markersGroup=null,this.userLocationMarker=null,this.userLocationCircle=null,this.userLat=null,this.userLng=null,this.lastLat=null,this.lastLng=null,this.heading=0,this.smoothedHeading=0,this.isAutoFollow=!0,this.watchId=null}async init(){if(!document.getElementById(this.mapContainerId))return;this.defaultZoom=17.5,this.map=L.map(this.mapContainerId,{preferCanvas:!0,zoomControl:!0,attributionControl:!0,zoomAnimation:!0,fadeAnimation:!0,markerZoomAnimation:!0,inertia:!0,inertiaDeceleration:3e3,easeLinearity:.25}).setView([-6.1754,106.8272],this.defaultZoom),L.tileLayer(`https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png`,{maxZoom:22,maxNativeZoom:19,attribution:`&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>`}).addTo(this.map),this.markersGroup=L.layerGroup().addTo(this.map),this.userLocationGroup=L.featureGroup().addTo(this.map),this.map.on(`dragstart`,()=>{this.setAutoFollow(!1)}),this.map.on(`zoomend viewreset resize`,()=>{if(this.userLat!==null&&this.userLng!==null&&this.userLocationCircle){let e=L.latLng(this.userLat,this.userLng);this.userLocationCircle.setLatLng(e),typeof this.userLocationCircle.redraw==`function`&&this.userLocationCircle.redraw()}});let e=document.getElementById(`recenter-gps-btn`);e&&e.addEventListener(`click`,()=>{this.recenterUser()}),this.startCompassListener(),this.startGpsTracking(),await this.refreshMarkers(),window.discoverPlantFromMap=async e=>{try{let t={plant_sighting_id:e};this.userLat!==null&&this.userLng!==null&&(t.latitude=this.userLat,t.longitude=this.userLng);let n=await window.apiClient.post(`/map/sightings/${e}/claim`,t),r=n.data||n;alert(r.message||`Selamat! Spesies tumbuhan berhasil kamu temukan dan masuk ke album Seedex!`),await this.refreshMarkers()}catch(e){alert(`Gagal mengklaim temuan: `+(e.response?.data?.message||e.message))}}}updateHeading(e){if(e==null||isNaN(e))return;let t=(e-this.smoothedHeading+540)%360-180;if(Math.abs(t)<2)return;this.smoothedHeading=(this.smoothedHeading+t*.2+360)%360,this.heading=this.smoothedHeading;let n=document.querySelector(`.user-gps-heading-arrow`);n&&(n.style.transform=`rotate(${Math.round(this.heading)}deg)`)}startCompassListener(){let e=e=>{let t=null;e.webkitCompassHeading!==void 0&&e.webkitCompassHeading!==null?t=e.webkitCompassHeading:e.alpha!==void 0&&e.alpha!==null&&(t=(360-e.alpha)%360),t!==null&&!isNaN(t)&&this.updateHeading(t)};window.DeviceOrientationEvent&&(typeof DeviceOrientationEvent.requestPermission==`function`?DeviceOrientationEvent.requestPermission().then(t=>{t===`granted`&&window.addEventListener(`deviceorientation`,e,!0)}).catch(console.warn):window.addEventListener(`deviceorientationabsolute`,e,!0)||window.addEventListener(`deviceorientation`,e,!0))}setAutoFollow(e){this.isAutoFollow=e;let t=document.getElementById(`recenter-gps-label`),n=document.getElementById(`recenter-gps-btn`),r=window.translations||{};t&&(t.textContent=e?r.auto_follow_on||`Pelacakan Otomatis Aktif`:r.auto_follow_off||r.recenter_gps||`Aktifkan Pelacakan Otomatis`),n&&(e?(n.classList.add(`bg-[#1F3D20]`,`border-[#F5F4DA]/40`),n.classList.remove(`bg-gray-800/80`,`border-gray-500/40`)):(n.classList.remove(`bg-[#1F3D20]`,`border-[#F5F4DA]/40`),n.classList.add(`bg-gray-800/80`,`border-gray-500/40`)))}recenterUser(){if(this.setAutoFollow(!0),this.userLat!=null&&this.userLng!=null&&this.map){let e=this.map.getZoom(),t=e&&e>=15?e:this.defaultZoom||17.5;this.map.flyTo([this.userLat,this.userLng],t,{animate:!0,duration:.8})}}calculateHeading(e,t,n,r){if(e==null||t==null||n==null||r==null)return 0;let i=(r-t)*Math.PI/180,a=e*Math.PI/180,o=n*Math.PI/180,s=Math.sin(i)*Math.cos(o),c=Math.cos(a)*Math.sin(o)-Math.sin(a)*Math.cos(o)*Math.cos(i);return(Math.atan2(s,c)*180/Math.PI+360)%360}startGpsTracking(){if(!navigator.geolocation)return;let e={enableHighAccuracy:!0,timeout:15e3,maximumAge:0},t=!1,n=e=>{let n=e.coords.latitude,r=e.coords.longitude,i=e.coords.heading;if(i!==null&&!isNaN(i)&&i!==0)this.updateHeading(i);else if(this.lastLat!==null&&this.lastLng!==null){let e=this.calculateDistanceMeters(this.lastLat,this.lastLng,n,r);if(e&&e>1.2){let e=this.calculateHeading(this.lastLat,this.lastLng,n,r);this.updateHeading(e)}}if(this.userLat=n,this.userLng=r,!t&&this.map&&(this.map.setView([n,r],this.defaultZoom||17.5),t=!0),this.addUserMarker(n,r),this.lastLat!==null&&this.lastLng!==null){let e=this.calculateDistanceMeters(this.lastLat,this.lastLng,n,r);e&&e>3&&this.refreshMarkers()}this.lastLat=n,this.lastLng=r};navigator.geolocation.getCurrentPosition(e=>{n(e),this.refreshMarkers()},e=>console.warn(`Akses lokasi GPS ditolak/tidak tersedia:`,e.message),e),this.watchId=navigator.geolocation.watchPosition(e=>{n(e)},e=>console.warn(`GPS Watch Error:`,e.message),e)}addUserMarker(e,t){if(!this.map||e==null||t==null)return;let n=this.userLat===null||this.userLng===null;this.userLat=e,this.userLng=t;let r=window.translations||{},i=r.gps_active||`GPS Presisi Aktif`,a=Math.round(this.heading||0),o=L.latLng(e,t),s=L.divIcon({className:`user-gps-marker`,html:`
        <div style="position:relative;width:40px;height:40px;display:flex;align-items:center;justify-content:center;">
          ...
          <div class="user-gps-heading-arrow" style="position:absolute;inset:0;transform:rotate(${a}deg);transition:transform 0.2s ease-out;pointer-events:none;display:flex;align-items:center;justify-content:center;">
      ...
      `,iconSize:[40,40],iconAnchor:[20,20]});if(this.userLocationGroup||=L.featureGroup().addTo(this.map),this.userLocationCircle)this.userLocationCircle.setLatLng(o);else{let e=r.claim_radius_title||`🎯 Zona Jangkauan Klaim Spesies (50 Meter)`;this.userLocationCircle=L.circle(o,{radius:50,color:`#10B981`,fillColor:`#10B981`,fillOpacity:.25,weight:3,dashArray:`5, 5`}).bindPopup(`<b style="font-family:'Baloo 2',sans-serif;font-size:12px;color:#065F46;">${e}</b>`),this.userLocationGroup.addLayer(this.userLocationCircle)}if(this.userLocationMarker){this.userLocationMarker.setLatLng(o);let e=document.querySelector(`.user-gps-heading-arrow`);e?e.style.transform=`rotate(${a}deg)`:this.userLocationMarker.setIcon(s)}else this.userLocationMarker=L.marker(o,{icon:s,zIndexOffset:1e3}).bindPopup(`<b style="font-family:Baloo 2,sans-serif;">📍 ${i}</b>`),this.userLocationGroup.addLayer(this.userLocationMarker);this.isAutoFollow&&this.map&&(this.lastLat===null||this.calculateDistanceMeters(this.lastLat,this.lastLng,e,t)>1.5)&&this.map.setView(o,this.map.getZoom(),{animate:!1}),n&&this.refreshMarkers()}async refreshMarkers(){if(this.markersGroup){this.markersGroup.clearLayers();try{let e=this.userLat!==null&&this.userLng!==null?`?lat=${this.userLat}&lng=${this.userLng}`:``,t=await window.apiClient.get(`/plant-sightings/nearby${e}`);(Array.isArray(t)?t:t.data||[]).forEach(e=>{this.addSightingMarker(e)})}catch(e){console.warn(`Gagal memuat marker temuan:`,e.message)}}}calculateDistanceMeters(e,t,n,r){if(e==null||t==null||n==null||r==null)return null;let i=(n-e)*Math.PI/180,a=(r-t)*Math.PI/180,o=Math.sin(i/2)*Math.sin(i/2)+Math.cos(e*Math.PI/180)*Math.cos(n*Math.PI/180)*Math.sin(a/2)*Math.sin(a/2);return 6371e3*(2*Math.atan2(Math.sqrt(o),Math.sqrt(1-o)))}addSightingMarker(e){if(!this.map||!this.markersGroup||!e.latitude||!e.longitude)return;let t=window.translations||{},n=e.species?e.species.common_name:`Tumbuhan Nyata`,r=e.species?e.species.species_code:`FLORA`,i=e.photo_url||e.species&&e.species.reference_image_url||(e.photo_path?`/storage/${e.photo_path.replace(/^\//,``)}`:``),a=e.sudah_ditemukan,o=[`ranger`,`admin`].includes(this.userRole),s=(t.mystery_plant||`Tanaman Misterius`).replace(/^❓\s*/,``),c=o||a?n:s,l=a?`#1F3D20`:o?`#8B6A4C`:`#D96C63`,u=`
      <div style="background-color:#FBFAF0;border:2px solid ${l};padding:3px 10px 3px 6px;border-radius:9999px;font-family:'Baloo 2',sans-serif;font-size:11px;font-weight:bold;color:${l};box-shadow:0 3px 8px rgba(0,0,0,0.2);white-space:nowrap;display:inline-flex;align-items:center;gap:5px;max-width:210px;box-sizing:border-box;cursor:pointer;">
        ${i&&(o||a)?`<img src="${i}" onerror="this.onerror=null; this.src='/images/logo-plantGuardian.jpeg';" style="width:20px;height:20px;border-radius:9999px;object-fit:cover;flex-shrink:0;border:1px solid ${l};" />`:`<span style="flex-shrink:0;">${a?`🌿`:o?`📍`:`❓`}</span>`}
        <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:150px;display:inline-block;">${c}</span>
      </div>
    `,d=L.divIcon({className:`gg-map-marker`,html:u,iconSize:null,iconAnchor:[60,14]}),f=``;if(this.userRole===`viewer`){let o=t.verified_badge||`Spesies Terverifikasi`,c=t.discover_button||`✨ Temukan & Klaim!`,l=t.already_discovered||`✓ Sudah Ditemukan`,u=t.unclaimed_badge||`🔒 Belum Diklaim`,d=t.unclaimed_tree||`Pohon ini belum diklaim! Tekan tombol di bawah untuk membuka dan mengklaim.`,p=this.calculateDistanceMeters(this.userLat,this.userLng,parseFloat(e.latitude),parseFloat(e.longitude)),m=``,h=!0,g=c,_=t.view_detail||`Detail`;if(p!==null){let e=Math.round(p);if(e>50){h=!1;let n=(t.distance_too_far||`📍 Jarak: :dist m (Maks 50m)`).replace(`:dist`,e),r=(t.distance_too_far_btn||`🔒 Terlalu Jauh (:dist m > 50m)`).replace(`:dist`,e);m=`<div style="font-size:10px;color:#DC2626;font-weight:bold;margin-bottom:6px;text-align:center;background-color:#FEE2E2;padding:2px 6px;border-radius:9999px;">${n}</div>`,g=r}else m=`<div style="font-size:10px;color:#16A34A;font-weight:bold;margin-bottom:6px;text-align:center;background-color:#DCFCE7;padding:2px 6px;border-radius:9999px;">${(t.distance_in_range||`📍 Jarak: :dist m (Dalam Jangkauan)`).replace(`:dist`,e)}</div>`}else m=`<div style="font-size:10px;color:#D97706;font-weight:bold;margin-bottom:6px;text-align:center;background-color:#FEF3C7;padding:2px 6px;border-radius:9999px;">${t.distance_enable_gps||`📍 Aktifkan GPS untuk mengklaim (Maks 50m)`}</div>`;f=`
        <div style="font-family:Nunito,sans-serif;max-width:220px;color:#2A2A22;padding:4px;box-sizing:border-box;">
          ...
          ${a?`<div style="width:100%;height:110px;border-radius:12px;overflow:hidden;margin-bottom:8px;background-color:#1F3D20;border:1.5px solid rgba(31,61,32,0.15);">
               ...
               </div>`:`<div style="background-color:rgba(226,225,196,0.4);border:1.5px dashed #8B6A4C;border-radius:12px;padding:10px 12px;text-align:center;margin-bottom:8px;font-size:11px;color:#6B6B55;font-style:italic;word-break:break-word;overflow-wrap:break-word;box-sizing:border-box;">
                ${d}
               </div>`}
      ...
      `}else{let r=t.edit_data_button||`✏️ Edit Data Tumbuhan`,a=t.status_label||`Status`,o=e.ranger?.name||e.ranger_name||`Ranger`,s=window.CURRENT_USER_ID,c=(this.userRole||`RANGER`).toUpperCase(),l=this.userRole===`admin`||s&&e.ranger_id==s;f=`
        <div style="font-family:Nunito,sans-serif;max-width:215px;color:#2A2A22;padding:4px;box-sizing:border-box;">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px;gap:4px;">
            <span style="background-color:#8B6A4C;color:#F5F4DA;font-family:'Baloo 2',sans-serif;font-size:10px;font-weight:bold;padding:1px 6px;border-radius:9999px;white-space:nowrap;">${c} SIGHTING</span>
            <span style="font-size:10px;color:#6B6B55;font-weight:bold;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:110px;" title="Pengunggah: ${o}">👤 ${o}</span>
          </div>
          ...
          <div style="font-size:10.5px;color:#6B6B55;margin:0 0 6px 0;line-height:1.3;">
            <span style="display:block;">${a}: <strong style="color:#1F3D20;">${e.verification_status}</strong></span>
            <span style="display:block;">Ranger Pengunggah: <strong style="color:#8B6A4C;">${o}</strong></span>
          </div>
          ...
          <div style="display:flex;flex-direction:column;gap:5px;margin-top:6px;">
            ...
            ${l?`<button onclick="window.openEditSightingModal(${e.id})" style="width:100%;background-color:#8B6A4C;color:#F5F4DA;font-family:'Baloo 2',sans-serif;font-weight:bold;font-size:12px;padding:6px 0;border-radius:9999px;border:none;cursor:pointer;box-shadow:0 3px 6px rgba(0,0,0,0.15);">
                  ${r}
