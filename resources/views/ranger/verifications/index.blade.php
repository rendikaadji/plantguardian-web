...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-[#5C574C]/30 pb-4">
        <div>
            ...
            <h1 class="font-serif-headline text-2xl font-bold" style="font-family: 'Fraunces', Georgia, serif; color: #2F4A3C !important;">
                Antrean Verifikasi Temuan Viewer
            </h1>
            <p class="text-xs text-[#5C574C]">
                Tinjau foto hasil scan AR dan bukti penanaman pohon nyata. Tandai "Setuju" atau "Tolak" untuk menjaga integritas data.
            </p>
        </div>
    </div>
    ...
    <div class="flex border-b border-[#5C574C]/30 gap-6">
        <button id="tab-sightings" onclick="switchTab('sightings')" class="pb-2 font-serif-headline text-sm font-bold border-b-2 border-[#2F4A3C] text-[#2F4A3C] cursor-pointer" style="font-family: 'Fraunces', Georgia, serif;">
            Hasil Scan AR (<span id="count-sighting-badge">0</span>)
        </button>
        <button id="tab-plantings" onclick="switchTab('plantings')" class="pb-2 font-serif-headline text-sm font-bold border-b-2 border-transparent text-[#5C574C] hover:text-[#2F4A3C] cursor-pointer" style="font-family: 'Fraunces', Georgia, serif;">
            Bukti Tanam Pohon Nyata (<span id="count-planting-badge">0</span>)
        </button>
    </div>
...
<script type="module">
    ...
    let currentTab = 'sightings';
    let queueData = { pending_sightings: [], pending_real_plantings: [] };
    ...
    async function loadQueue() {
        ...
        try {
            ...
            queueData = res.data || { pending_sightings: [], pending_real_plantings: [] };

            document.querySelector('#count-sighting-badge').textContent = queueData.pending_sightings.length;
            document.querySelector('#count-planting-badge').textContent = queueData.pending_real_plantings.length;
    ...
    window.switchTab = (tab) => {
        currentTab = tab;
        const btnS = document.querySelector('#tab-sightings');
        const btnP = document.querySelector('#tab-plantings');

        if (tab === 'sightings') {
            btnS.className = 'pb-2 font-serif-headline text-sm font-bold border-b-2 border-[#2F4A3C] text-[#2F4A3C] cursor-pointer';
            btnP.className = 'pb-2 font-serif-headline text-sm font-bold border-b-2 border-transparent text-[#5C574C] hover:text-[#2F4A3C] cursor-pointer';
        } else {
            btnP.className = 'pb-2 font-serif-headline text-sm font-bold border-b-2 border-[#2F4A3C] text-[#2F4A3C] cursor-pointer';
            btnS.className = 'pb-2 font-serif-headline text-sm font-bold border-b-2 border-transparent text-[#5C574C] hover:text-[#2F4A3C] cursor-pointer';
        }
    ...
    function renderTab() {
        const container = document.querySelector('#queue-container');
        const list = currentTab === 'sightings' ? queueData.pending_sightings : queueData.pending_real_plantings;
        ...
        if (list.length === 0) {
            container.innerHTML = `
                <div class="p-8 text-center border border-dashed border-[#5C574C]/40 rounded-xs" style="background-color: #E3DABF !important;">
                    ...
                    <p class="text-xs text-[#5C574C]">Tidak ada temuan yang memerlukan peninjauan di kategori ini.</p>
                </div>
            `;
            return;
        }

        if (currentTab === 'sightings') {
            ...
            container.innerHTML = list.map(item => `
                <div class="p-4 border border-[#5C574C]/30 rounded-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-xs" style="background-color: #EDE6D3 !important;">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="space-y-1">
                            <span class="font-mono-code text-[10px] uppercase text-[#9C6644] font-bold" style="font-family: 'IBM Plex Mono', monospace;">
                                PENANAM: ${item.user?.name || 'Viewer'}
                            </span>
                            <h3 class="font-serif-headline text-base font-bold text-[#2A2823]" style="font-family: 'Fraunces', Georgia, serif;">
                                ${item.plant_species ? item.plant_species.common_name : 'Penanaman Pohon Nyata'}
                            </h3>
                            <span class="font-mono-code text-[10px] text-[#5C574C] block" style="font-family: 'IBM Plex Mono', monospace;">
                                Tanggal Tanam: ${new Date(item.planted_at || item.created_at).toLocaleString('id-ID')}
                            </span>
                        </div>
                    </div>

                    <div class="flex items-center gap-2 border-t md:border-t-0 pt-3 md:pt-0 border-[#5C574C]/20 shrink-0 w-full md:w-auto justify-end">
                        <button onclick="decidePlanting(${item.id}, 'verified')" class="px-4 py-2 bg-[#2F4A3C] text-[#EDE6D3] font-mono-code text-xs font-bold rounded-xs hover:bg-[#2A2823] cursor-pointer" style="font-family: 'IBM Plex Mono', monospace;">VERIFIKASI</button>
                        <button onclick="decidePlanting(${item.id}, 'rejected')" class="px-4 py-2 border border-[#8B3A3A] text-[#8B3A3A] font-mono-code text-xs font-bold rounded-xs hover:bg-[#8B3A3A] hover:text-[#EDE6D3] cursor-pointer" style="font-family: 'IBM Plex Mono', monospace;">TOLAK</button>
                    </div>
                </div>
            `).join('');
        }
    ...
    window.decidePlanting = async (id, status) => {
        try {
            await apiClient.post(`/ranger/verifications/real-plantings/${id}`, { status });
            alert(`Penanaman pohon berhasil ditandai ${status}.`);
            await loadQueue();
        } catch (err) {
            alert('Gagal memverifikasi: ' + err.message);
        }
